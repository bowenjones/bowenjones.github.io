...
station <- read.csv(file = "./Duplication_Project_files/2016_station_data.csv", header = TRUE, 
    stringsAsFactors = FALSE)

trip <- read.csv(file = "./Duplication_Project_files/2016_trip_data.csv", header = TRUE, 
    stringsAsFactors = FALSE)

weather <- read.csv(file = "./Duplication_Project_files/2016_weather_data.csv", header = TRUE, 
    stringsAsFactors = FALSE)
```
...
```{r}
Trip_Duration_Month %>%
  mutate(Season = ifelse(ym %in% c("2014/10", "2014/11"), "Fall",
                                            ifelse(ym %in% c("2014/12", "2015/1", "2015/2"), "Winter",
                                            ifelse(ym %in% c("2015/3", "2015/4", "2015/5"), "Spring", "Summer")))) %>% 
  group_by(ym, Season) %>%
ggplot(aes(ym, Avg, color = Season)) +
  geom_point() +
...
trip_seasons %>%
  ...
  summarize(N = n()) %>%
  ggplot(aes(Hour, N, color = season, group = season)) +
  geom_point() +
  geom_line() + 
...
trip_seasons %>%
  ...
  filter(usertype == "Short-Term Pass Holder" | usertype == "Member") %>% 
  summarize(N = n()) %>%
ggplot(aes(ym, N, group = usertype)) +
  geom_point(aes(shape = usertype, color = season, size = 3)) +
...
start_date_ym %>% 
  mutate(trip_duration_min = tripduration/60) %>% 
  group_by(usertype, trip_duration_min) %>%
    filter(usertype == "Short-Term Pass Holder" | usertype == "Member") %>% 
ggplot(aes(trip_duration_min)) +
  geom_histogram(color = "blue", fill = "lightblue") +
  geom_vline(aes(xintercept = 45), lty = 2) +
  geom_vline(aes(xintercept = 30), lty = 2) +
  xlim(0, 60) +
...
```{r}
trip_cost %>% 
  group_by(tripduration_m, usertype, cost) %>%
  filter(usertype == "Short-Term Pass Holder" | usertype == "Member") %>% 
  filter(cost != "0") %>% 
  ggplot(aes(cost)) +
...
```{r}
weather %>%
  group_by(Date, Min_TemperatureF) %>% 
...
```{r}
weather %>%
  group_by(Date, Mean_Temperature_F) %>% 
...
```{r}
weather %>%
  group_by(Date, Max_Temperature_F) %>% 
...
```{r}
trip_weather$Mean_Temperature_F <- mround(trip_weather$Mean_Temperature_F, 5)
trip_weather %>% 
  group_by(Mean_Temperature_F) %>%
  summarize(N = n()) %>%
  ggplot(aes(x = Mean_Temperature_F, y = N)) +
  geom_line() + 
...
```{r}
 trip_weather %>% 
  group_by(Mean_Temperature_F) %>%
  summarize(N = n()) %>% 
  ggplot(aes(x = Mean_Temperature_F, y = length(Mean_Temperature_F)/N)) +
  geom_line() + 
...
```{r}
trip_weather$Precipitation_In <- mround(trip_weather$Precipitation_In, 0.2)

trip_weather %>% 
  group_by(Precipitation_In) %>%
  summarize(N = n()) %>% 
ggplot(aes(Precipitation_In, N)) +
  geom_line() +
  theme_bw()
